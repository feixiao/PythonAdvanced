...
**就绪(Ready):** 进程已准备好执行，但还未分配到 CPU 时间片。它等待系统调度程序将其分配给一个可用的处理器。

**运行(Running):** 进程当前正在执行指令，并且占用了 CPU 时间片。
...
所谓的 pid 是计算机上运行的程序都有一个序列号，我们称之为 pid 可以通过 os 模块 os.getpid()获取到
